...
The legacy case specification remains `runtime_ready=false` and contract-only. The separate workflow layer reads curated CER public facts and a hash-bound derivative of the completed frozen Step 2B assessment. Evidence snapshots are immutable and content-addressed; checkpoints append monotonically within each workflow; allowlisted preferences are a separate, intentionally mutable current-session document. Completed session replay is bound to the snapshots and creates no new checkpoint or evidence file. It does not run EO processing, modify the historical run, consume approval, or turn a registry or Safeguard fact into an empirical, compliance, credit-quality, tokenisation, or financial conclusion.
