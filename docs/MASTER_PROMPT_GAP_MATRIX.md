...
| R4 | B0/B1/T1 and provider path | adapter and 27 matched B1/T1 packet pairs implemented; behavioural status `NOT_EXECUTED`, 27 planned and 0 completed per arm | `NOT_EXECUTED_BLOCKED_NO_CREDENTIAL_AND_MODEL_AUTHORIZATION` |
| R5 | Framework ADR | repository-local requirements and re-evaluation triggers; external links are informational and no version-specific fact is relied upon | `COMPLETE` |
...
| R14 | H1 independent human evaluation | status `NOT_EXECUTED`; 27 labels planned, 0 completed; guide exists | `NOT_EXECUTED_PENDING_INDEPENDENT_ANNOTATORS` |
| R15 | Live CER API and application hosting | curated public pages/CSV only; no stable CER API contract; no service deployed | `BLOCKED_EXTERNAL_INTERFACE_AND_DEPLOYMENT_DECISIONS` |
